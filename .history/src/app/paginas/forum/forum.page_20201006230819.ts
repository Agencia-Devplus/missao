...
export class ForumPage {
  ...
  likeButton(idPergunta) {
    if (this.likeSource === this.likedPressed) {
      this.crudService.like(idPergunta, this.user.uid).then(() => {
        this.crudService.likeThatShit(idPergunta, this.user.uid);
      });
    } else {
      this.crudService.dislike(idPergunta, this.user.uid);
    }
  }
    ...
    try {
      this.crudService.read_Perguntas().subscribe((data) => {
        ...
        this.array.forEach((item) => {
          this.crudService.isLiked(item.id).subscribe((data) => {
            this.likeSource = data.curtidas_usuarios.includes(this.user.uid)
              ? "../../../assets/icon/thumbs-up.svg"
              : "../../../assets/icon/thumbs-up-outline.svg";
          });
