...
export class CrudService {
  ...
  isLiked(postagem){
   return this.firestore.collection("Perguntas").doc(postagem).valueChanges();   
  }

  likeThatShit(postagem, userUid){
    this.firestore.collection("Perguntas").doc(postagem).update({
      curtidas_usuarios: firebase.firestore.FieldValue.arrayUnion(userUid)
    });
  }
}
